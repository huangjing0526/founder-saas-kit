...
### Added
- **Evolvability: interface contract.** New `STABILITY.md` declares what's stable for downstream (public npm-script names, the guard stdin/exit protocol, the quality-script CLI + graceful-skip behavior, skill frontmatter keys, the installer CLI + manifest schema) vs. what's internal — i.e. what SemVer means for this kit. New `contract.test.cjs` (`npm run test:contract`, wired into `harness`) asserts all of §1, so breaking a downstream contract fails CI here instead of silently breaking adopters. Also fixed `install.mjs --help` to exit 0 (was a usage-error exit when no `--target`).
