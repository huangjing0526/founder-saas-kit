...
| **engineering/** `methodology/` | `anti-drift`(防漂移五原则,最高价值)/ `spec-driven-development`(SDD 闭环 + 规范↔代码追溯)/ `distilling-from-real-projects`(kit 如何从真实项目长大)/ `north-star-rules` / `common-pitfalls` / 事故台账骨架 | 通用 |
